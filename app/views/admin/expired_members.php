...
<div class="admin-content">
    <div class="container-fluid">
        <div class="page-head">ManageExpired Member - ALL</div>
        ...
        <div class="top-controls">
            <div class="controls-row controls-row-top cm-top-row">
                ...
                <div class="cm-actions-wrap text-end">
                    ...
                    <button class="btn btn-info text-white me-2" type="button" onclick="openFilterPopup()"><i class="bi bi-funnel"></i> Filter</button>
            ...
            <ul class="nav nav-tabs custom-tabs mt-3">
                <li class="nav-item"><a class="nav-link <?= ($filter ?? 'all') === 'all' ? 'active' : '' ?>" href="<?= BASE_URL ?>/admin/expired-members?status_filter=all">All <small>(<?= (int)count($users) ?>)</small></a></li>
                <li class="nav-item"><a class="nav-link <?= ($filter ?? 'all') === 'approved' ? 'active' : '' ?>" href="<?= BASE_URL ?>/admin/expired-members?status_filter=approved">Approved List <small>(<?= (int)$approvedCount ?>)</small></a></li>
                <li class="nav-item"><a class="nav-link <?= ($filter ?? 'all') === 'unapproved' ? 'active' : '' ?>" href="<?= BASE_URL ?>/admin/expired-members?status_filter=unapproved">Unapproved List <small>(<?= (int)$unapprovedCount ?>)</small></a></li>
            </ul>
        </div>
            ...
            <div class="user-card searchable-card" data-date="<?= strtotime($u['created_at']) ?>" data-name="<?= strtolower($u['first_name'].' '.$u['last_name']) ?>">
                ...
                <div class="user-main-content">
                    ...
                    <div class="details-column details-grid">
                        ...
                        <p><strong>Plan Expired On</strong><span>:</span> Expired</p>
                    ...
                    <div class="details-column details-grid">
                        ...
                        <p><strong>Final Rishta Fee</strong><span>:</span> 0</p>
                    </div>
                </div>
...
<div id="planModalOverlay" class="plan-modal-overlay" style="display:none;">
    <div class="plan-modal-box">
        <div class="plan-modal-head">
            <h4>Plan Assignment</h4>
            <button type="button" class="plan-close-btn" onclick="closePlanModal()">&times;</button>
        </div>
        <form id="planAssignForm">
            <input type="hidden" name="user_id" id="plan_user_id">
            ...
            <div class="plan-user-strip">
                ...
                <div class="plan-user-meta">
                    ...
                    <div id="plan_user_badge" class="plan-user-badge">Member</div>
                    <div id="plan_user_email"><i class="fa fa-envelope"></i> -</div>
                    <div id="plan_user_phone"><i class="fa fa-phone"></i> -</div>
                    <div id="plan_user_id_label"><i class="fa fa-tag"></i> -</div>
                </div>
            </div>
            <div class="plan-grid">
                <label>Plan</label><select id="plan_name" class="form-control"><option value="">Select Plan</option><option value="basic">Basic</option><option value="premium">Premium</option><option value="gold">Gold</option></select>
                <label>Plan Amount</label><input type="number" id="plan_amount" class="form-control" value="0">
                <label>Staff</label><select id="plan_staff" class="form-control"><option value="">Select Staff</option><option>Ali Jawad</option><option>Kiran Deo Jawad</option></select>
                <label>Team</label><select id="plan_team" class="form-control"><option value="">Select Team</option><option>Marketing</option><option>Support</option></select>
                <label>Final Rishta Fee</label><input type="number" id="plan_final_fee" class="form-control" value="50000">
                <label>Bonus Days</label><input type="number" id="plan_bonus_days" class="form-control" value="0">
                <label>Discount</label><input type="number" id="plan_discount" class="form-control" value="0">
                <label>Description</label><textarea id="plan_description" class="form-control" rows="3" placeholder="Enter Payment Note"></textarea>
            </div>
            <div class="plan-modal-foot">
                <button type="submit" class="btn btn-info text-white btn-sm">Submit</button>
                <button type="button" class="btn btn-light btn-sm border" onclick="closePlanModal()">Close</button>
            </div>
        </form>
    </div>
</div>
...
<div id="filterPopup" class="custom-popup-overlay" style="display:none;">
    <div class="custom-popup">
        ...
        <div class="popup-body"><div class="form-group"><label>Status</label><select class="form-control" id="statusFilter"><option value="">All</option><option value="approved">Approved</option><option value="unapproved">Unapproved</option></select></div></div>
...
<style>
    ...
    .controls-row .btn-danger{background:#ef4c5a;border-color:#ef4c5a}.controls-row .btn-info{background:#44c0df;border-color:#44c0df}.controls-row .btn-primary{background:#0e98d3;border-color:#0e98d3}
    .controls-row .input-group{display:flex;flex-wrap:nowrap;width:100%}.controls-row .input-group .form-control{height:34px;font-size:12px;border-color:#d8d8d8;min-width:0;flex:1 1 auto}
    ...
    .btn-action{padding:6px 12px;border:none;color:#fff;border-radius:2px;text-decoration:none;font-size:11px;min-width:110px;text-align:center}.lightblue{background:#54c3da}.dark{background:#34495e}.yellow-btn{background:#efc145;color:#6f5100}
    ...
    .plan-modal-overlay{position:fixed;inset:0;background:rgba(18,26,39,.62);display:flex;align-items:center;justify-content:center;z-index:1070}
    .plan-modal-box{width:700px;max-width:94%;background:#fff;border-radius:3px;border:1px solid #d8d8d8;box-shadow:0 18px 40px rgba(0,0,0,.22)}
    .plan-modal-head{display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:1px solid #ececec}
    .plan-modal-head h4{margin:0;font-size:21px;font-size:20px;font-weight:500;color:#666}
    .plan-close-btn{border:0;background:transparent;font-size:22px;color:#9aa3ad;line-height:1}
    .plan-user-strip{display:flex;gap:14px;padding:12px 14px 10px;border-bottom:1px solid #f0f0f0}
    .plan-user-strip img{width:84px;height:84px;object-fit:cover;border:1px solid #ddd}
    .plan-user-meta{font-size:12px;color:#666;display:flex;flex-direction:column;gap:4px;justify-content:center}
    .plan-user-name{font-weight:700;color:#4d4d4d}
    .plan-user-badge{display:inline-block;background:#2f3742;color:#fff;font-size:10px;padding:2px 6px;border-radius:2px;width:max-content}
    .plan-grid{display:grid;grid-template-columns:150px 1fr;gap:9px 14px;padding:12px 14px}
    .plan-grid label{font-size:12px;color:#444;align-self:center}
    .plan-grid .form-control{height:32px;font-size:12px;border-color:#ddd}
    .plan-grid textarea.form-control{height:auto;min-height:64px}
    .plan-modal-foot{padding:10px 14px 14px;display:flex;gap:8px;justify-content:center}
...
function applyFilterStatus(){const st=document.getElementById('statusFilter').value;const target=st?`<?= BASE_URL ?>/admin/expired-members?status_filter=${encodeURIComponent(st)}`:`<?= BASE_URL ?>/admin/expired-members`;window.location.href=target;}
...
function openPlanModal(data){document.getElementById('plan_user_id').value=data.userId||'';document.getElementById('plan_package_id').value=data.packageId||'';document.getElementById('plan_user_name').textContent=`${data.name||'-'} - ${data.matri||''}`;document.getElementById('plan_user_badge').textContent=data.name||'Member';document.getElementById('plan_user_email').innerHTML=`<i class="fa fa-envelope"></i> ${data.email||'-'}`;document.getElementById('plan_user_phone').innerHTML=`<i class="fa fa-phone"></i> ${data.phone||'-'}`;document.getElementById('plan_user_id_label').innerHTML=`<i class="fa fa-map-marker"></i> Lives in Rawalpindi`;document.getElementById('plan_final_fee').value=data.fee||30000;document.getElementById('plan_user_img').src=data.image||'<?= BASE_URL ?>/assets/images/default-avatar.png';document.getElementById('planModalOverlay').style.display='flex';}
function closePlanModal(){document.getElementById('planModalOverlay').style.display='none';}
document.getElementById('planAssignForm')?.addEventListener('submit',function(e){e.preventDefault();closePlanModal();});
updateList();
</script>
