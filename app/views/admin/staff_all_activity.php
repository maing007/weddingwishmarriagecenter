<?php
$title = 'Staff All Activity';
$reportTotal = count($activityRows);
...
<div class="admin-topbar sa-topbar">
    <div class="admin-topbar-left">
        ...
        <div class="admin-topbar-title">Manage Activity - ALL</div>
    </div>
    ...
    <div class="container-fluid">
        <div class="sa-report-panel">
            <div class="sa-toolbar d-flex flex-wrap align-items-stretch justify-content-between gap-2 mb-3">
                <div class="sa-search-block flex-grow-1" style="min-width: 280px; max-width: 520px;">
                    <div class="input-group sa-search-group">
                        <input type="text" id="saSearch" class="form-control sa-input" placeholder="Search here...">
                        <button class="btn btn-light border sa-clear flex-shrink-0" type="button" id="saClearSearch" aria-label="Clear"><i class="fa fa-times"></i></button>
                        <button class="btn sa-btn-search flex-shrink-0" type="button" id="saSearchBtn"><i class="bi bi-search me-1"></i> Search</button>
                    </div>
                </div>
                <div class="sa-actions flex-shrink-0 d-flex align-items-stretch gap-2">
                    ...
                    <button type="button" class="btn sa-btn-filter d-flex align-items-center" id="saOpenFilter"><i class="bi bi-funnel me-1"></i> Filter</button>
                </div>
            </div>
            ...
            <div class="sa-footer mt-3 pt-3 pb-4 d-flex flex-wrap align-items-center justify-content-between gap-2">
                <div class="small text-muted" id="saInfo">Showing 0 to 0 of 0 entries</div>
                <nav class="sa-pagination-wrap" id="saPaginationWrap" style="display:none;" aria-label="Table pages">
                    <ul class="pagination pagination-sm mb-0" id="saPagination"></ul>
                </nav>
            </div>
        </div>
    </div>
...
<div id="saFilterPopup" class="sa-popup-overlay" style="display:none;">
    <div class="sa-popup">
        <div class="sa-popup-header"><h3 class="sa-popup-title">Filter</h3><span class="sa-popup-close" id="saCloseFilter">&times;</span></div>
        <div class="sa-popup-body">
            <div class="mb-3">
                <label class="form-label small">Staff Name</label>
                <select id="saFilterStaff" class="form-select form-select-sm">
            ...
            <div class="mb-3">
                ...
                <select id="saFilterDept" class="form-select form-select-sm">
            ...
            <div class="mb-3">
                <label class="form-label small">Main Topic</label>
                <select id="saFilterTopic" class="form-select form-select-sm">
        ...
        <div class="sa-popup-footer">
            <button type="button" class="btn sa-btn-search btn-sm" id="saApplyFilter">Apply</button>
            <button type="button" class="btn btn-secondary btn-sm" id="saCancelFilter">Close</button>
        </div>
    </div>
</div>
...
<script>
(function(){
    var tbody = document.getElementById('saTbody');
    var searchInput = document.getElementById('saSearch');
    var showSel = document.getElementById('saShowEntries');
    ...
    function rows() { return Array.from(document.querySelectorAll('.sa-row')); }
    ...
    document.getElementById('saSortDate') && document.getElementById('saSortDate').addEventListener('click', function() {
    ...
    document.getElementById('saSearchBtn') && document.getElementById('saSearchBtn').addEventListener('click', resetPage);
    document.getElementById('saClearSearch') && document.getElementById('saClearSearch').addEventListener('click', function() {
        searchInput.value = '';
        resetPage();
    });
    showSel && showSel.addEventListener('change', function() { currentPage = 1; render(); });

    document.getElementById('saOpenFilter') && document.getElementById('saOpenFilter').addEventListener('click', function() {
        document.getElementById('saFilterPopup').style.display = 'flex';
    });
    function closeFilter() { document.getElementById('saFilterPopup').style.display = 'none'; }
    document.getElementById('saCloseFilter') && document.getElementById('saCloseFilter').addEventListener('click', closeFilter);
    document.getElementById('saCancelFilter') && document.getElementById('saCancelFilter').addEventListener('click', closeFilter);
    document.getElementById('saApplyFilter') && document.getElementById('saApplyFilter').addEventListener('click', function() {
