...
<div class="admin-content">
    <div class="container-fluid">
        <div class="page-head">ManageEdit Member Plan - ALL</div>
        ...
        <div class="top-controls">
            ...
            <div class="controls-row controls-row-bottom cm-bottom-row mt-3">
                ...
                <div class="cm-sort-wrap text-end">
                    <div class="sort-wrap">
            ...
            <ul class="nav nav-tabs custom-tabs mt-3">
                <li class="nav-item"><a class="nav-link <?= ($filter ?? 'all') === 'all' ? 'active' : '' ?>" href="<?= BASE_URL ?>/admin/change-membership-plan?status_filter=all">All <small>(<?= (int)count($users) ?>)</small></a></li>
                <li class="nav-item"><a class="nav-link <?= ($filter ?? 'all') === 'approved' ? 'active' : '' ?>" href="<?= BASE_URL ?>/admin/change-membership-plan?status_filter=approved">Approved List <small>(<?= (int)$approvedCount ?>)</small></a></li>
                <li class="nav-item"><a class="nav-link <?= ($filter ?? 'all') === 'unapproved' ? 'active' : '' ?>" href="<?= BASE_URL ?>/admin/change-membership-plan?status_filter=unapproved">Unapproved List <small>(<?= (int)$unapprovedCount ?>)</small></a></li>
            </ul>
        </div>
            ...
            <div class="user-card searchable-card" data-date="<?= strtotime($u['created_at']) ?>" data-name="<?= strtolower($u['first_name'].' '.$u['last_name']) ?>">
                ...
                <div class="action-row">
                    <form method="post" action="<?= BASE_URL ?>/admin/users/send-email-confirmation">
                        ...
                        <button class="btn-action blue" type="submit">Confirm email</button>
                    </form>
    ...
    <div class="plan-modal-box">
        ...
        <form id="planAssignForm">
            ...
            <div class="plan-grid">
                ...
                <input type="number" id="plan_amount" class="form-control" value="0">

                <label>Staff</label>
                <select id="plan_staff" class="form-control">
                    <option value="">Select Staff</option>
                    <option>Ali Jawad</option>
                    <option>Kiran Deo Jawad</option>
                </select>

                <label>Team</label>
                <select id="plan_team" class="form-control">
                    <option value="">Select Team</option>
                    <option>Marketing</option>
                    <option>Support</option>
                </select>

                <label>Final Rishta Fee</label>
                <input type="number" id="plan_final_fee" class="form-control" value="50000">

                <label>Bonus Days</label>
                <input type="number" id="plan_bonus_days" class="form-control" value="0">

                <label>Discount</label>
                <input type="number" id="plan_discount" class="form-control" value="0">

                <label>Description</label>
                <textarea id="plan_description" class="form-control" rows="3" placeholder="Enter Payment Note"></textarea>
            </div>
...
<style>
    ...
    .plan-modal-overlay{position:fixed;inset:0;background:rgba(0,0,0,.58);display:flex;align-items:center;justify-content:center;z-index:1070}
    .plan-modal-box{width:680px;max-width:94%;background:#fff;border-radius:4px;border:1px solid #d8d8d8;box-shadow:0 12px 30px rgba(0,0,0,.2)}
    ...
    .plan-modal-head h4{margin:0;font-size:22px;font-size:20px;font-weight:500;color:#666}
    .plan-close-btn{border:0;background:transparent;font-size:22px;color:#999;line-height:1}
    .plan-user-strip{display:flex;gap:12px;padding:12px 14px 8px;border-bottom:1px solid #f0f0f0}
    .plan-user-strip img{width:80px;height:80px;object-fit:cover;border:1px solid #ddd}
    ...
    .plan-grid .form-control{height:32px;font-size:12px}
    .plan-grid textarea.form-control{height:auto}
    ...
    @media(max-width:640px){.plan-grid{grid-template-columns:1fr}.plan-grid label{margin-top:2px}}
</style>
...
function updateList(){
    ...
    const filtered = cards.filter((card) => card.innerText.toLowerCase().includes(search));
...
function applyFilterStatus(){ const st = document.getElementById('statusFilter').value; const target = st ? `<?= BASE_URL ?>/admin/change-membership-plan?status_filter=${encodeURIComponent(st)}` : `<?= BASE_URL ?>/admin/change-membership-plan`; window.location.href = target; }
...
function openPlanModal(data){
    document.getElementById('plan_user_id').value = data.userId || '';
    document.getElementById('plan_package_id').value = data.packageId || '';
    document.getElementById('plan_user_name').textContent = `${data.name || '-'} - ${data.matri || ''}`;
    document.getElementById('plan_user_email').innerHTML = `<i class="fa fa-envelope"></i> ${data.email || '-'}`;
...
document.getElementById('planAssignForm')?.addEventListener('submit', function(e){
    e.preventDefault();
    closePlanModal();
});
updateList();
